...
RSpec.describe Solution do
  ...
  it 'gives the correct solution for the example' do
    ...
    expect(bingo.last_board_final_call).to eq 13
    expect(bingo.last_board_id).to eq 1
    expect(bingo.last_board_uncalled_sum).to eq 148
    expect(bingo.result).to eq 1924
  end
end
