class Solution
  ...
  def final_equation
    return @final_equation if defined? @final_equation
  ...
  def find_largest_magnitude
    @final_equation, @final_sum =
      *homework
        .map { |x| SnailfishNumber.new x }
        .combination(2)
        .map { |(x, y)| [[[x,y], x + y], [[y,x], y + x]] }
        .flatten(1)
        .max_by { |(equation, answer)| answer.magnitude }
  end
