...
RSpec.describe Solution do
  ...
  describe Solution::LanternFish do
    it 'starts with a spawn_timer of 8' do
      expect(described_class.new.spawn_timer).to eq 8
    end

    it 'can be created with an explicit spawn_timer value' do
      expect(described_class.new(spawns_in: 5).spawn_timer).to eq 5
    end

    it 'decrements spawm when a day elapses' do
      lantern_fish = described_class.new(spawns_in: 8)
      expect {
        lantern_fish.day_elapsed!
      }.to change {
        lantern_fish.spawn_timer
      }.from(8).to(7)
    end

    it 'resets spwan timer to 6 when a day elapses if the timer was 0 at the start of the day' do
      lantern_fish = described_class.new(spawns_in: 0)
      expect {
        lantern_fish.day_elapsed!
      }.to change {
        lantern_fish.spawn_timer
    ...
    it 'spawns a child at the end of a day if the spawn_timer is 0' do
      lantern_fish = described_class.new(spawns_in: 1)
      expect(lantern_fish.children.size).to eq 0

      expect { lantern_fish.day_elapsed! }.not_to change { lantern_fish.children.size }

      expect(lantern_fish.spawn_timer).to eq 0

      expect { lantern_fish.day_elapsed! }.to change { lantern_fish.children.size }.from(0).to(1)
    end

    it 'spawns a child with spawn timer of 8 when it spawns a child' do
      lantern_fish = described_class.new(spawns_in: 0)

      lantern_fish.day_elapsed!
      expect(lantern_fish.children.first.spawn_timer).to eq 8
    end
  end
end
