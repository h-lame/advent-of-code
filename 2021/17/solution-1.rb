class Solution
  ...
  def firing_plan
    return @firing_plan if defined? @firing_plan

    @firing_plan = valid_x_velocities.map do |x|
      valid_y_velocities.map do |y|
        FiringPlan.new(x, y, target_area).tap { |fp| fp.execute! }
      end
    end.flatten.select { |fp| fp.hit_target? }.max_by { |fp| fp.max_height }
  end

  def result
    firing_plan.max_height
  end
  ...
  def valid_x_velocities
    ...
    (1..).lazy.each do |x|
      sum = (x * (x + 1))/2
      puts "Looking at #{x} - #{sum}"
      next if sum < target_area[:x].min
      break if sum > target_area[:x].max
      valid << x
    end
    valid
  end
  ...
  def valid_y_velocities
    ...
    (1..target_area[:y].min.abs).each do |y|
      up = (y * (y + 1))/2
