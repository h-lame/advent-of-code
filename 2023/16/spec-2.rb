...
RSpec.describe Solution2 do
  ...
  describe described_class::MirrorGrid do
    ...
    it 'energizes the grid correctly' do
      expect(described_class.new(['...']).render_energized(described_class::Beam.new([0,0], :east))).to eq ['###']

      expect(described_class.new([
         '.',
         '.'
      ]).render_energized(described_class::Beam.new([0,0], :east))).to eq([
         '#',
         '.'
      ])
    ...
    it 'can detect the best state by choosing the start row or column' do
      grid = described_class.new(raw_mirror_grid)

      expect(grid.best_possible).to eq 51
      expect(grid.render_best_possible).to eq([
        '.#####....',
        '.#.#.#....',
        '.#.#.#####',
        '.#.#.##...',
  ...
  it 'gives the correct solution for the examples' do
    expect(described_class.new(raw_mirror_grid).result).to eq 51
  end
end
