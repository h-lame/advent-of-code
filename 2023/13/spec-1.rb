...
RSpec.describe Solution1 do
  ...
  describe described_class::Pattern do
    ...
    it 'can calculate the score for a pattern - lines before a vertial reflection or 100 * lines before a horizontal reflection' do
      expect(described_class.from_raw(raw_patterns[0]).score).to eq 5
      expect(described_class.from_raw(raw_patterns[1]).score).to eq 400
    end
  end

  it 'gives the correct solution for the examples' do
    expect(described_class.new(raw_patterns).result).to eq 405
  end
end
