...
RSpec.describe Solution1 do
  ...
  describe described_class::History do
    it 'calculates the correct next value' do
      expect(described_class.new(raw_readings[0]).next_reading).to eq 18
      expect(described_class.new(raw_readings[1]).next_reading).to eq 28
      expect(described_class.new(raw_readings[2]).next_reading).to eq 68
    end
  ...
  it 'gives the correct solution for the examples' do
    expect(described_class.new(raw_readings).result).to eq 114
  end
end
