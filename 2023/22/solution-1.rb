...
class Solution1
  ...
  Brick = Data.define(:id, :top, :tail) do
    ...
    def cross_over?(other) = Solution1.range_overlap?(self.x_span, other.x_span) && Solution1.range_overlap?(self.y_span, other.y_span)

    def fall(other_bricks)
      return self if z_span.min == 1
      equal_or_below = other_bricks.reject { |ob| ob.above? self }
      supporting = equal_or_below.select { |ob| ob.supports? self }.max_by { |ob| ob.z_span.max }
  ...
  class Jenga
    def initialize(raw_bricks)
      @bricks = raw_bricks.map.with_index { |(top, tail), idx| Brick.new(id: Solution1.to_base39(idx), top: XYZ.new(*top), tail: XYZ.new(*tail)) }
    ...
    def settle!
      ...
      bricks.sort_by { |b| b.z_span.min }.each do |brick|
  ...
  def result
    jenga.settle!
    rb = jenga.removable_bricks
    puts "Reckon I can remove #{rb}"
    rb.size
  end
end
