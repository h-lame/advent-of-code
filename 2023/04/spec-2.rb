...
RSpec.describe Solution2 do
  ...
  describe Solution2::Card do
    ...
    it 'tells us the ids of the prize cards' do
      expect(described_class.generate_from(raw_scratchcards[0]).prizes).to eq [2, 3, 4, 5]
      expect(described_class.generate_from(raw_scratchcards[1]).prizes).to eq [3, 4]
      expect(described_class.generate_from(raw_scratchcards[2]).prizes).to eq [4, 5]
      expect(described_class.generate_from(raw_scratchcards[3]).prizes).to eq [5]
      expect(described_class.generate_from(raw_scratchcards[4]).prizes).to eq []
      expect(described_class.generate_from(raw_scratchcards[5]).prizes).to eq []
    end
  end

  it 'gives the correct solution for the example' do
    expect(described_class.new(raw_scratchcards).result).to eq 30
  end
end
